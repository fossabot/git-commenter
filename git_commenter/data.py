...
class DataLoader:
    ...
    def load_emojis(self) -> list:
        ...
        """
        return sorted(
            self.data["emoji"], key=lambda x: x["frequency"], reverse=True
        )
    ...
    def load_verbs(self) -> list:
        ...
        """
        return sorted(
            self.data["verb"], key=lambda x: x["frequency"], reverse=True
        )
    ...
    def load_objects(self) -> list:
        ...
        """
        return sorted(
            self.data["object"], key=lambda x: x["frequency"], reverse=True
        )
    ...
    def load_templates(self) -> list:
        ...
        """
        return sorted(
            self.data["template"], key=lambda x: x["frequency"], reverse=True
        )

    def store_history(
        self, emoji=None, verb=None, object_=None, template=None
    ):
        if emoji is not None:
            for emoji_d in self.load_emojis():
                if emoji_d["emoji"] == emoji:
                    emoji_d["frequency"] += 1

        if verb is not None:
            for verb_d in self.load_verbs():
                if verb_d["verb"] == verb:
                    verb_d["frequency"] += 1

        if object_ is not None:
            for object_d in self.load_objects():
                if object_d["object"] == object_:
                    object_d["frequency"] += 1

        if template is not None:
            for template_d in self.load_templates():
                if template_d["template"] == template:
                    template_d["frequency"] += 1
